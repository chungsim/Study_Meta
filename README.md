...
2. 오브젝트별 기능
    ...
    텍스트는 3D 오브젝트의 TMP를 사용하였습니다.
    이로 인해 orderInLayer를 통한 관리가 힘들어 수정이 필요하다고 생각합니다.


3. 느낀점
    이번 개인과제를 통해 기본적인 시스템을 구현하는 실력이 많이 부족하다는 것을 알게 되었습니다.
    UI 관리와 오브젝트 상호작용은 가장 필수적인 시스템이고 3D로 넘어가면 구현이 더욱 복잡해지는 만큼 이번 과제를 진행하면서 생긴 의문점들을 정리하고 최적의 해결방법을 찾는 시간을 가져야겠다고 느꼈습니다. 
